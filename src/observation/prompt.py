...
3. **深度等级按证据强度分级估计，cm 仅在可靠参照时填**：
   - 有清晰参照（水位线、已知尺寸物体的明确比例）→ 给精确区间并给相应置信；
   - 只有中等参照（路面标线、车胎、水波等）→ 给宽区间，`depth_estimate.confidence` 用 low；
   - 几乎无参照但积水明显 → **保守单值等级估计**（按可见视觉强度取保守档，如 L2/L3），`depth_estimate.confidence=low`，`depth_cm` 保持 null——cm 承载测量，无可靠参照不编造厘米数；
   - 仅当**积水存在与否本身存疑**（极端模糊 / 夜间无光 / 镜头污渍遮挡 / 信号互相矛盾）→ 才用 LX。
4. 只有存在清晰水位线或已知尺寸参照与水面的明确关系时，才填 depth_cm，并优先给区间而非伪精确单值；**无可靠参照时 depth_cm 全 null**。
5. 至少两个相互支持的视觉证据才能给 medium/high 置信度；证据矛盾时扩大区间并降低置信度。
